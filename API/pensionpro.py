...
#headers = {'username': 'kelafs', 'apikey': '1FDL5D64j2sn1cWykcs93v68'}
...
def get_worktray(name:str, get_all:bool=False, addfilt:str=None, n:int=75, dashboard:bool=False, simp:bool=False, dtf:bool=False, orderby=None, top=None, skip=None):
    ...
    if dtf:
        dtfcols = dtf if isinstance(dtf, list) else ['task_active', 'eff_on', 'proj_due_on', 'per_start', 'per_end', 'ip_eff_on', 'add_on']
        for col in dtfcols:
            dfw[col] = pd.to_datetime(dfw[col], format='%m/%d/%Y %I:%M:%S %p')
            if col in ['per_end', 'per_start']:
                dfw[col] = dfw[col].dt.normalize()
